package main

import (
	"context"
	"log"
	"os/signal"
	"runtime/debug"
	"sync"
	"syscall"
	"time"

	"github.com/gofiber/fiber/v2"
	"github.com/gofiber/fiber/v2/middleware/cors"
	"github.com/yusufekoanggoro/flight-search-system/main-service/delivery"
	shared "github.com/yusufekoanggoro/flight-search-system/main-service/shared/sse"
	"github.com/yusufekoanggoro/flight-search-system/main-service/usecase"
	"github.com/yusufekoanggoro/flight-search-system/pkg/redisclient"
	"github.com/yusufekoanggoro/flight-search-system/pkg/redisclient/stream"
)

func main() {
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()

	defer func() {
		if r := recover(); r != nil {
			log.Printf("\x1b[31;1mFailed to start service: %v\x1b[0m\n", r)
			log.Printf("Stack trace: \n%s\n", debug.Stack())
		}
	}()

	redisConfig := &redisclient.Config{
		Addr:     "localhost:6379",
		Password: "",
		DB:       0,
	}

	rdb, err := redisclient.New(ctx, redisConfig)
	if err != nil {
		log.Fatal(err)
	}

	redisStreamConfig := stream.Config{
		Group:        "main-service",
		Consumer:     "consumer-1",
		Streams:      []string{"flight.search.results"},
		Count:        10,
		Block:        5 * time.Second,
		NoAck:        false,
		GroupStartID: "0", // ketika buat group, baca dari awal stream
		ReadStartID:  ">", // baca pesan yang belum di-ack (baru)
	}
	redisStreamConsumer := stream.NewConsumerGroup(&redisStreamConfig, rdb.Client())

	redisStream := stream.NewClient(rdb.Client())

	hub := shared.NewHub()

	usecase := usecase.NewFlightUsecase(redisStream)
	resthandler := delivery.NewRestHandler(usecase, hub)
	redisHandler := delivery.NewRedisHandler(redisStreamConsumer, hub)

	fiberApp := fiber.New()
	fiberApp.Use(cors.New(cors.Config{
		AllowOrigins: "*",
		AllowHeaders: "Cache-Control, Content-Type",
	}))
	group := fiberApp.Group("/api/flights")

	resthandler.RegisterRoutes(group)
	redisHandler.RegisterSubscribers()

	wg := &sync.WaitGroup{}

	wg.Add(1)
	go func() {
		defer wg.Done()
		hub.Run()
	}()

	wg.Add(1)
	go func() {
		defer wg.Done()
		if err := redisStreamConsumer.StartConsuming(ctx); err != nil {
			log.Fatalf("Redis Stream consumer error: %v", err)
		}
	}()

	go func() {
		if err := fiberApp.Listen(":3000"); err != nil {
			log.Printf("Fiber app terminated: %v", err)
		}
	}()

	log.Println("Server is running on :3000")

	<-ctx.Done()
	log.Println("Context canceled, shutting down services...")

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	fiberApp.ShutdownWithContext(ctx)
	hub.Close()
	rdb.Close()

	wg.Wait()
	log.Println("App exited")
}
